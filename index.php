...
$infoUser = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM users_info WHERE id='$id'"));
...
<body>
    ...
    <header class="header">
        <div class="header__container">
            <img src="<?= $infoUser['avatar'] ?>" alt="" class="header__img">
                    ...
                    <tbody>
                        ...
                        if ($topic != NULL) {
                            foreach ($topic as $result) {
                                echo '<tr>';
                                echo "<td style='text-align: left; vertical-align: middle;'><span style='font-size:80%'>Раздел:{$result["category_name"]} </span> <br> <strong> <a href='/question.php?topicID={$result["topic_id"]}&page=1'>{$result["topic_title"]}</a></strong>  </td>";
                                ...
                                echo "<td style='position:relative'><form style='position: absolute; width: 100%; height: 100%; top: 0; left: 0;' method='post' action='vendor/deleteTopic.php'><input type='hidden' name='topic-id' value='" . $result["topic_id"] . "'><input style='width:100%; height:100%; border-color: #bfbfff; background: #febfff; border-radius:5px' type='submit' value='УДАЛИТЬ'></form></td>";
                                echo '</tr>';
                            }
                        } else {
                            echo '<tr>';
                            echo "<td colspan='5' style='text-align: center; vertical-align: middle;'>Вы никаких тем не создавали</td>";
                            echo '</tr>';
                        }
                        ?>
                    </tbody>
